...
async function seedTestData() {
  try {
    ...
    for (const concepto of conceptos) {
      const { data } = await supabase.from('conceptos').insert({
        usuario_id: userId,
        ...concepto,
      }).select();
      if (data) conceptosCreados.push(data[0]);
    }
    ...
    for (let i = 2; i >= 0; i--) {
      ...
      const { data } = await supabase.from('periodos').insert({
        usuario_id: userId,
        mes,
        anio,
